...
/**
 * Uploaded source PDFs (see `pdf-ingestion` module), keyed by project id;
 * each value is a plain object keyed by sourceId so a single `PersistedMap`
 * (not a nested one) can hold every uploaded PDF for a project — a project
 * usually has a handful of PDFs, so rewriting the whole object on every
 * upload is cheap and keeps this consistent with every other store here.
 */
export const sourcePdfsByProject = new PersistedMap<Record<string, SourcePdfRecord>>({
  storeKey: "sourcePdfsByProject",
});
