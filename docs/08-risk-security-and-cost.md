...
| RSK-05  | PDF parse sai hoặc không đọc được      | Likelihood Cao; impact Cao                    | Evidence offset/provenance sai                                 | Encrypted/malformed PDF; text mismatch; page extraction empty    | MIME/size/page validation, `pdfjs-dist`/`pdf-parse` page model, exact-text check, representative PDF spike | Abstract/manual evidence with provenance; document limitation; revisit a Python worker only through a follow-up ADR if Node extraction blocks P0 | Backend Lead + AI/Evidence Lead     | `PLANNED` |
| RSK-06  | LLM structured output lỗi              | Likelihood Cao; impact Trung bình             | Job fail hoặc corrupt data                                     | Invalid JSON/schema/enum/ID                                      | Zod validation, ID allowlist, schema-specific prompts, bounded repair                                      | Terminal error + regenerate/user path; never guess fields/IDs                                                                                    | AI/Evidence Lead                    | `PLANNED` |
...
| RSK-13  | Input/reference validation thiếu       | Chưa có score; impact Cao                     | Cross-project link, corrupt graph, unauthorized data reference | Hallucinated ID, invalid relation/offset, schema bypass          | API authority, Zod/tRPC contracts, DB constraints, same-project checks, transactions                       | Reject/rollback, preserve error code, add regression fixture                                                                                     | Backend Lead                        | `PLANNED` |
...
| RSK-16  | Queue integration chậm                 | Likelihood Trung bình; impact Trung bình      | Platform work lấn feature                                      | Redis/BullMQ work blocks P0 stories                              | P0 job abstraction and in-process execution                                                                | Defer Redis/BullMQ to P1; keep explicit job status                                                                                               | Backend Lead                        | `PLANNED` |
...
- The Node.js tRPC/Fastify API validates all external input and owns authorization/domain transitions.
...
4. Reject encrypted/malformed documents when the selected Node PDF parser cannot safely parse.
...
- Zod/tRPC input-output schemas, enum and length validation.
